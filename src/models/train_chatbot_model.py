...
from build_features import create_train_test
...
os.chdir(f"./speaking_with_plato/models/")


class Args:
    def __init__(self):
        self.output_dir = "./small-dialouGPT/"
...
class ConversationDataset(Dataset):
    def __init__(self, tokenizer: PreTrainedTokenizer, args, df, block_size=512):
        ...
        block_size = block_size - (
            # fix the issues where GPT2TokenizerFast doesn't have "max_len" attribute
...
def load_and_cache_examples(args, tokenizer, train_df, test_df, evaluate=False):
    return ConversationDataset(tokenizer, args, test_df if evaluate else train_df)
...
def evaluate(
    ...
    eval_dataset = load_and_cache_examples(
        args, tokenizer, train_df, test_df, evaluate=True
    )
...
def main(train_df, test_df):
    ...
    if args.do_train:
        train_dataset = load_and_cache_examples(
            args, tokenizer, train_df, test_df, evaluate=False
        )
    ...
    if args.do_eval and args.local_rank in [-1, 0]:
        ...
        for checkpoint in checkpoints:
            ...
            model.to(args.device)
            result = evaluate(args, model, tokenizer, train_df, test_df, prefix=prefix)
...
if __name__ == "__main__":
    train_df, test_df = create_train_test(context_n=20)
